...
namespace postKCGL
{
    ...
    public sealed partial class MySqlHelper
    {
        ...
        /// <returns></returns>
        public DataTable GetEmptyDataTable(string dataTableName)
        {
            using (MySqlConnection connection = new MySqlConnection(ConnectionString))
            {
                //MySqlParameter msp = new MySqlParameter("?tablename",MySqlDbType.String);
                //msp.Value = dataTableName;
                //return GetDataTable("select * from ?tablename where 1=0", msp);
                return GetDataTable("select * from "+dataTableName+" where 0=1");
        ...
        public DataTable GetDataTable(string commandText)
        {
            using (MySqlConnection connection = new MySqlConnection(ConnectionString))
            {
                return GetDataTable(connection, null, CommandType.Text, commandText, 0, 0, null);
            }
        }
        public DataTable GetDataTable(string commandText, int min, int max)
        {
            using (MySqlConnection connection = new MySqlConnection(ConnectionString))
            {
                return GetDataTable(connection, null, CommandType.Text, commandText, min, max, null);
            }
        }
        public DataTable GetDataTable(string commandText,params MySqlParameter[] parms)
        {
            using (MySqlConnection connection = new MySqlConnection(ConnectionString))
            {
                return GetDataTable(connection, null, CommandType.Text, commandText, 0, 0, parms);
            }
        }
        public DataTable GetDataTable(MySqlConnection connection, MySqlTransaction transaction, CommandType commandType, string commandText, int min, int max, params MySqlParameter[] parms)
        {
            ...
            if (commandText.IndexOf("@") > 0)
            {
                ...
                if (index > 0)
                {
                    dt.ExtendedProperties.Add("SQL", commandText.Substring(0, index - 1));  //将获取的语句保存在表的一个附属数组里，方便更新时生成CommandBuilder
                }
                else
                {

                    dt.ExtendedProperties.Add("SQL", commandText);  //将获取的语句保存在表的一个附属数组里，方便更新时生成CommandBuilder
                }
            }
            else
            {
        ...
        /// <summary>
        ///大批量数据插入,返回成功插入行数,DataTable.TableName为mysql表名
        /// </summary>
        /// <param name="connectionString">数据库连接字符串</param>
        /// <param name="table">数据表</param>
        /// <returns>返回成功插入行数</returns>
